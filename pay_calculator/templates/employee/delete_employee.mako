		...
		<table >
			...
			<tr v-for="row in search_result">
				...
				<td style="cursor: pointer;color:blue;">
					<!-- <a :href="'/employee_delete?id='+row['employee_id']" target="_blank"> --><span @click="delete_employee(row['employee_id'])">delete</span>
				</td>
			</tr>
		</table>
	</div>
...
<script type="text/javascript">
	var app = new Vue({
		...
		methods: {
			...
			delete_employee: function(emp_id){
				var url = '/delete_employee_from_database?id='+emp_id;
				axios.get(url).then(function(result){
					alert(result['data']);
					app.fetch_employee();
				})
